...
class Map:
    ...
    def scale_up(self, key=None):
        k = 2
        spn = tuple(map(lambda x: x / k, self.params['spn']))
        if spn[0] >= 0.0001953125 and spn[1] >= 0.0001953125:
            self.params['spn'] = spn
            self.update_ui()
            self.on_search()
    ...
    def update_data(self):
        ...
        spn = self.string_to_coord(self.spn_input.get_text())
        if spn[0] <= 52.2 and spn[1] <= 52.2 and spn[0] >= 0.0001953125 and spn[1] >= 0.0001953125:
